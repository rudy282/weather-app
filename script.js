...
window.onload = function (e) {
	...
	if (!firstVisit) {
		...
		if (citiesLoaded != null)
			for (const i of citiesLoaded) addNewPlaceToTheList(i);
		if (currentLocation == "true" || currentLocation == null)
			getCurrentLocation(coordinates);
		else getGeoCoords(startingLocation);
		createMap(map);
	} else {
		e.preventDefault();
		showSettingsMenu();
...
const list_element_current_location = document.querySelector(
	"#list-element-current-location"
);
list_element_current_location.addEventListener("click", () => {
	getCurrentLocation(coordinates);
});
...
add_button.addEventListener("click", (e) => {
	...
	checkIfExists(searchValue, addNewPlaceToTheList);
});

const checkIfExists = async (searchQuery, callback) => {
	try {
		const response = await fetch(
			`https://api.openweathermap.org/geo/1.0/direct?q=${searchQuery}&appid=${APIKey}`
		);
...
function addNewPlaceToTheList(searchValue) {
	...
	newValue.firstChild.addEventListener("click", () => {
...
function getWeatherForecastData(coords) {
	...
	fetch(
		...
		.catch((error) => {
			showPopup(
				"Sorry, we cannot connect to the server, you have connectivity issues."
			);
			console.error(error);
		});
}
...
function getGeoCoords(searchQuery) {
	...
	fetch(
		...
		.catch((error) => {
			showPopup("Sorry, we couldn't find this place, please change it.");
			console.error(error);
		})
		.catch(() => {
			return false;
...
function changeData(data) {
	...
	try {
		...
		console.error("There is a problem with chart.js CDNs");
	}
...
function changeWeatherInfo(data) {
	...
	if (window.width > 950) {
		if (
			Math.round(data.list[0].main.temp) < -9 ||
			data.city.name.length > 20
		)
			weather.style.width = "355px";
...
function changeWeatherForecastInfo(data) {
	...
	for (let i = 1; i < 6; i++) {
		let forecastWeatherIcon =
			data.list[datesArrIndexes[i - 1]].weather[0].icon;
		document.querySelector(
			"#forecast-day-img-" + i
		).src = `https://openweathermap.org/img/wn/${forecastWeatherIcon}.png`;
		document.querySelector("#forecast-day-degree-data-" + i).innerHTML =
			Math.round(data.list[datesArrIndexes[i - 1]].main.temp) + "&#176;C";
		document.querySelector("#forecast-day-degree-weekday-" + i).innerHTML =
			epochToJsDate(
				data.list[datesArrIndexes[i - 1]].dt
			).toLocaleDateString("us", { weekday: "long" });
	}

	try {
		const precipitation_chart_button = document.querySelector(
			"#precipitation-chart-button"
		);
		precipitation_chart_button.addEventListener("click", () =>
			updateChart(data, true)
		);
		const temperature_chart_button = document.querySelector(
			"#temperature-chart-button"
		);
		temperature_chart_button.addEventListener("click", () => {
			updateChart(data, false);
		});
	} catch (error) {
		console.error("There is a CDN error");
	}
}
...
function changeChartData(data, isPrecipitation) {
	...
	for (let i = 0; i < DATA_LENGTH; i++) {
		if (i < 8) {
			weatherDataPrecipitation[
				epochToJsDate(data.list[i].dt).getHours() - 1 + ":00"
			] = data.list[i].pop * 100;
			...
			weatherDataPrecipitation[
				epochToJsDate(data.list[i].dt).getHours() - 1 + ":00 next day"
			] = data.list[i].pop;
			weatherDataTemperature[
				epochToJsDate(data.list[i].dt).getHours() - 1 + ":00 next day"
			] = Math.round(data.list[i].main.temp);
...
function showSettingsMenu() {
	...
	const settings_menu_current_location = document.querySelector(
		"#settings-menu-current-location"
	);
	settings_menu_current_location.value = currentLocation;

	const settings_menu_starting_location = document.querySelector(
		"#settings-menu-starting-location"
	);
...
function changeAdditionalWeatherInfo(data) {
	const additional_info_feels_like = document.querySelector(
		"#additional-info-feels-like"
	);
	const additional_info_pressure = document.querySelector(
		"#additional-info-pressure"
	);
	const additional_info_clouds = document.querySelector(
		"#additional-info-clouds"
	);

	const additional_info_visibility = document.querySelector(
		"#additional-info-visibility"
	);
	const additional_info_sunrise = document.querySelector(
		"#additional-info-sunrise"
	);
	const additional_info_sunset = document.querySelector(
		"#additional-info-sunset"
	);

	additional_info_feels_like.innerHTML = Math.round(
	...
	if (sunrise.getHours() < 10)
		additional_info_sunrise.innerHTML =
			"0" + sunrise.getHours() + ":" + sunrise.getMinutes();
	else
		additional_info_sunrise.innerHTML =
	...
	if (sunset.getHours() < 10)
		additional_info_sunset.innerHTML =
			"0" + sunset.getHours() + ":" + sunset.getMinutes();
	else
		additional_info_sunset.innerHTML =
